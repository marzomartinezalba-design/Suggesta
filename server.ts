...
app.use(express.json());
  ...
  app.post("/api/gemini/search", async (req, res) => {
    ...
    const aiInstance = serverAi || getAiClient();
    if (!aiInstance) {
      return res.status(500).json({ error: "Gemini server client is not initialized. Please configure GEMINI_API_KEY." });
    }
    try {
      ...
      console.error("Server searchItemsAI error:", error);
  ...
  app.post("/api/gemini/item-info", async (req, res) => {
    ...
    try {
      ...
      console.error("Server generateItemInfo error:", error);
  ...
  app.post("/api/gemini/reviews", async (req, res) => {
    ...
    try {
      ...
      console.error("Server generateCommunityReviews error:", error);
  ...
  app.post("/api/gemini/recommendations", async (req, res) => {
    ...
    try {
      ...
      console.error("Server generateCommunityRecommendations error:", error);
